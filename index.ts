		...
		try {
			...
			for(const { name, branch: refBranch, remote } of refs) {
				...
				if(!hashes.includes(hash)) {
					...
					/* await git.merge(["--commit", "--no-edit", "--allow-unrelated-histories", `${remote}/${branch}`]).catch(err => {
						conflict = true;
						console.error("Ref \"%s\" has a merge conflict, this must be fixed on github.", name);
					});
					await git.add(".");
					await git.commit(`Update ref "${name}""`); */
					...
					const pr = await octo.request("POST /repos/{owner}/{repo}/pulls", {
							...
							title: `Remote Update: ${remote}/${refBranch}`,
